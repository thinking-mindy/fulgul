...
lazy_static::lazy_static! {
    static ref TERMINAL_SESSIONS: Arc<TokioMutex<std::collections::HashMap<String, TerminalSession>>> = Arc::new(TokioMutex::new(std::collections::HashMap::new()));
}
...
pub async fn start_command_execution(
    ...
    // Prepare command with password if needed
    let _final_command = if command.contains("sudo") && password.is_some() {
        let pwd = password.unwrap();
        format!("echo '{}' | sudo -S {}", pwd, command.replace("sudo", "").trim())
    } else {
        command.clone()
    };
    ...
    #[cfg(target_os = "linux")]
    {
        ...
        TERMINAL_SESSIONS.lock().await.insert(session_id.clone(), session);
        
        // Spawn tasks to read stdout and stderr
        let sessions_clone_stdout = TERMINAL_SESSIONS.clone();
        let sessions_clone_stderr = TERMINAL_SESSIONS.clone();
        let sessions_clone_final = TERMINAL_SESSIONS.clone();
                ...
                match reader.read_line(&mut line).await {
                    ...
                    Ok(_) => {
                        ...
                        if !output_line.is_empty() {
                            if let Some(session) = sessions_clone_stdout.lock().await.get_mut(&session_id_stdout) {
            ...
            loop {
                ...
                match reader.read_line(&mut line).await {
                    ...
                    Ok(_) => {
                        let output_line = line.trim_end().to_string();
                        if !output_line.is_empty() {
                            if let Some(session) = sessions_clone_stderr.lock().await.get_mut(&session_id_stderr) {
                                session.output.push(format!("[stderr] {}", output_line));
                            }
                        }
                    }
                    Err(_) => break,
        ...
        tokio::spawn(async move {
            match child.wait().await {
                Ok(status) => {
                    ...
                    if let Some(session) = sessions_clone_final.lock().await.get_mut(&session_id_final) {
                        session.status = if exit_code == 0 { "completed".to_string() } else { "failed".to_string() };
...
pub async fn execute_command_interactive(
    ...
    // Keep this for backward compatibility, but use start_command_execution for streaming
    #[cfg(target_os = "linux")]
    {
        use tokio::process::Command;
        use std::process::Stdio;
